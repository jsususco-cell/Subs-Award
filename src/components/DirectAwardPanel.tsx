...
export default function DirectAwardPanel({
  ...
  const total = contract
    ...
    : categoriesTotal(fields.categories);
        ...
        {contract ? (
          ...
          <section className="overflow-hidden rounded-xl border border-navy-200 bg-white shadow-sm">
            <header className="border-b border-navy-100 px-4 py-3">
              ...
              <p className="mt-0.5 text-xs text-navy-600/70">
                Leave a category at zero where it does not apply — it is then
                left off the purchase order rather than written as $0.
              </p>
            </header>
              ...
              {CATEGORY_FIELDS.map((c) => (
                <div key={c.key}>
                  <span className="mb-1 block text-xs font-medium text-navy-700">
                    {c.label}
                    {c.hint && (
                      <span className="font-normal text-navy-600/60">
                        {" "}
                        ({c.hint})
                      </span>
                    )}
            ...
            <p className="border-t border-navy-100 px-4 py-2 text-xs text-navy-600/70">
              Quickbase computes Total Amount from exactly these seven
              categories, so this is the contract amount and the figure the
              bills divide.
            </p>

...
export const QB_CONFIG = {
  ...
  fields: {
    jobs: {
      ...
      /** "Canopy | Model Home Type" — prefills House on the purchase order. */
      house: Number(process.env.QB_JOB_HOUSE_FID ?? 1161),
    },
    vendors: {
      recordId: 3,
      company: 23,
...
export async function fetchJobs(
  ...
  if (f.house) select.push(f.house);
